...
export function Chip({ label, color, className }: { label: string; color?: string; className?: string }) {
  return (
    <span
      className={cn(
        "inline-flex h-5 max-w-40 items-center gap-1.5 truncate rounded-full border border-white/[0.08] bg-white/[0.04] px-2 text-[11px] font-medium whitespace-nowrap text-muted-foreground",
        className
      )}
    >
