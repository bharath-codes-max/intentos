...
// The host identifier ("claude-code") and its on-disk dotfolder convention (".claude") are
// NOT the same string — naively interpolating `.${HOST}` produced ".claude-code", which
// doesn't exist and silently failed to find a real, present token file. Mapped explicitly
// per host instead of assumed.
const HOST_DIR = {
  "claude-code": ".claude",
  cursor: ".cursor",
  codex: ".codex",
...
function resolveToken() {
  ...
  const hostDir = HOST_DIR[HOST] ?? `.${HOST}`;
  const candidates = [
    join(process.cwd(), hostDir, ".intentos-token"),
    join(process.cwd(), ".intentos", "token"),
    // Global connector installer writes here — one credential file for every project on
    // this machine, since the device-enrollment flow issues one token per device, not per repo.
    join(homedir(), ".intentos", "token"),
    join(homedir(), hostDir, "intentos-token"),
  ];
...
function resolveApiUrl() {
  if (process.env.INTENTOS_API_URL) return process.env.INTENTOS_API_URL;
  const configPath = join(homedir(), ".intentos", "config.json");
...
async function post(path, body) {
  if (!TOKEN) {
    const hostDir = HOST_DIR[HOST] ?? `.${HOST}`;
    throw new Error(
      `No Intentos token found (checked INTENTOS_TOKEN, ${hostDir}/.intentos-token, .intentos/token, ~/${hostDir}/intentos-token)`
    );
  }
...
async function main() {
  ...
  switch (eventName) {
    case "SessionStart": {
      // Claude Code's SessionStart carries `cwd`; other hosts may not — resolveProjectIdentity
      // returns {} for an absent cwd, so this stays a no-op for hosts that don't provide it.
      const project = resolveProjectIdentity(parsed.cwd);
      const metadata = Object.keys(project).length ? { project } : undefined;
      await bestEffort(() => post("/v1/runs", { provider: HOST, external_session_id: sessionId, metadata }));
      return;
    }
    case "UserPromptSubmit": {
      // Claude Code's field is `user_input` (confirmed against the current hooks reference);
      // other hosts sharing this script may send `prompt` — both are accepted.
      const promptText = parsed.user_input ?? parsed.prompt;
      if (typeof promptText === "string" && promptText.trim()) {
        await bestEffort(() =>
          post("/v1/runs/task", { provider: HOST, external_session_id: sessionId, prompt: promptText })
        );
      } else {
        process.exitCode = 0;
      }
