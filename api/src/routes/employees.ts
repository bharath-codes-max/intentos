...
export async function employeesRoutes(app: FastifyInstance) {
  app.get("/v1/employees", { preHandler: requireOrgAdmin }, async (req) => {
    ...
    return sql`
      select
        u.id, u.email, u.role, u.created_at, u.revoked_at,
  ...
  /** Disconnects every device this employee has enrolled, signs them out everywhere, and
   *  blocks them from signing back in — the user row itself stays, so their name/email
   *  still shows correctly on their past decisions instead of those rows going orphaned. */
  app.post("/v1/employees/:id/revoke", { preHandler: requireOrgAdmin }, async (req, reply) => {
    ...
    const [target] = await sql`select id, org_id, revoked_at from users where id = ${id}`;
    ...
    if (target.revoked_at) {
      return reply.code(400).send({ error: "This employee's access is already revoked" });
    }

    await sql`update users set revoked_at = now() where id = ${id}`;
